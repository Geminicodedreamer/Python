...
def train_dqn(env, num_episodes, gamma, epsilon, lr, memory_size=10000, batch_size=64, target_update=10):
    ...
    # 提取 i 和对应的计数
    # Re-order i and count values based on the original order of i
    i_values = list(default_counter.keys())
    count_values = list(default_counter.values())

    # Sort i_values to ensure they are in increasing order
    sorted_i_values = sorted(i_values)
    sorted_count_values = [default_counter[i] for i in sorted_i_values]

    # Plot the line chart
    plt.figure(figsize=(10, 6))
    plt.plot(sorted_i_values, sorted_count_values, marker='o', linestyle='-', color='b')
    plt.title('Count of Each Cell (Original Order)')
