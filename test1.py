...
env_train = CellSelectionEnvWithLSTM(
    train_data, error_bound=0.1, quality_threshold=212*0.9, lstm_model=lstm_model, seq_length=seq_length)
...
env_test = CellSelectionEnvWithLSTM(
    test_data, error_bound=0.1, quality_threshold=48*0.9, lstm_model=lstm_model, seq_length=seq_length)
...
# 绘制测试阶段选定单元格比例曲线
plt.plot(test_selected_cells_ratios[-48*100:])  # 只绘制后面的时间步
plt.xlabel('Time Step')
plt.ylabel('Selected Cells Ratio')
